...
set_role_color_by_csv_mark --> set color by csv file (format of device.yaml changed!)
check_device_role_yaml -> check if device_role and markdown are equal
set color --> set color by json file
...
def set_role_color_by_csv_mark():
    ...
    df_cl = pd.read_csv(
        COLOR_CSV, delimiter=";", dtype=defaultdict(lambda: str)
    )
    df_cl.darker = df_cl.darker.str.split(",")
    df_cl[["color_name", "choice"]] = df_cl[["color_name", "choice"]].apply(
    ...
    df_str[["name", "parent"]] = df_str[["name", "parent"]].apply(
        lambda col: col.str.lower()
    )
    pre_mrk = pd.DataFrame(
        data=df_str.loc[
            df_str.name.str.contains("::", na=False), "name"
    ...
    try:
        ...
        for ind in pre_mrk.index:
            df_cl.choice[df_cl.color_name == pre_mrk.loc[ind]["color"]] = (
                pre_mrk.loc[ind]["role"]
            )
            df_cl.notes[df_cl.color_name == pre_mrk.loc[ind]["color"]] = (
                "markdown color"
            )
    except ValueError:
    ...
    # set level 0
    df_str.loc[(df_str.level == 0) & (df_str.color.isna()), "color"] = (
        df_str.loc[(df_str.level == 0) & (df_str.color.isna())]["color"].map(
            lambda c: color_to_choice.pop(0)
        )
    )
    ...
    for level in range(1, df_str.level.max() + 1):
        if level == 1:
            ...
            df_str.loc[
                (df_str.level == level) & (df_str.color.isna()), "color"
            ] = df_str.loc[(df_str.level == level) & (df_str.color.isna())][
                "parent"
            ].map(
                lambda c, color_to_choice=color_to_choice: color_to_choice.get(
                    df_str.loc[df_str.name == c, "color"].iloc[0], np.nan
                )
            )
        else:
            color_to_choice = dict(
                zip(
                    df_cl["darker"].apply(lambda x, level=level: x[level - 2]),
                    df_cl["darker"].apply(
                        lambda x, level=level: x[level - 1 :]
                    ),
                )
            )
            for parent in df_str.loc[
                (df_str.level == level) & (df_str.color.isna())
            ]["parent"].unique():
                df_str.loc[
                    (df_str.level == level)
                    & (df_str.color.isna())
                    ...
                    df_str.loc[df_str.name == parent, "color"].iloc[0]
                ).pop(0)
    # Export dataframe to yaml
    with open(PATH_OUT, "w", encoding="utf-8") as file:
        yaml.dump(
            df_str.reset_index(drop=True).to_dict(orient="records"),
            file,
            sort_keys=False,
            indent=2,
        )
...
def get_structure():
    ...
    df_rl = df_rl.astype(
        {"name": str, "parent": str, "children": int, "level": int}
    )
    ...
    df_rl.loc[df_rl["name"].isin(counts.index), "children"] = df_rl["name"].map(
        counts
    )
